...
module controller (clk,rst,enable,in1,in2,prod);
    ...
    parameter integer pair_h1[0:6] = {0, 0,0,0,0,0,0};
    reg[1:3]pair_h;
//reg pair_h1[1:ST]={0,0,0};
    input clk;
    input rst;
    input enable;
    input  [N:1] in1;
  ...
  wire [N+5:3] o2;
  reg [(N+2)+1+2:1] stage3in;
  reg [N+5:1] st3_1, st3_2;
  wire [N+10:5] o3;
  reg [(N+5)+1+4:1] stage4in;
  
 reg  bit0;
 reg [10:1]stored;
 reg [13:1]stored2;
  //reg [N+9:1] st4_1, st4_2;
 // wire [N+18:9] o4;
//  reg [(N+9)+1+8:1] stage5in[0:1];
//  reg [N+19:1] st5_1, st5_2;
//  wire [ N+19+1+16:17] o5;
//  reg  [(N+19)+1+16:1] stage6in[0:0];
...
for(i=1;i<256;i=i+1)begin
	for(j=1;j<256;j=j+1)begin
		in1=i; in2=j; 
		#70 $display("Product=%0d",prod);$fwrite(fd,prod,"\t",in1,"\t",in2,"\n"); 
